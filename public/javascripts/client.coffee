...
class Game
  ...
  connected: => @send 'newplayer', @player.name

  disconnect: (options) => noGameScreen()
...
class Controller
  ...
  # TODO: Make sure the color changes on update, check if it's recieved properly in the Game.init method
  setColor: (color) ->
    @_color = color
    @_changeColor()
